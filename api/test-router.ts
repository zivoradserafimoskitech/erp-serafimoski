// ===== Test Router - End-to-End Testing =====
import { z } from "zod";
...
export const testRouter = createRouter({
  fullFlow: publicQuery
    .input(z.object({ materialQty: z.number().default(100) }))
    .mutation(async ({ input }) => {
      ...
      await db.insert(inventoryTransactions).values({
        ...
        quantity: String(receiptQty),
        unitCost: receiptPrice, totalCost: String(itemTotal),
      ...
      await db.update(workOrders)
        .set({ costAmount: String(materialCost), status: "completed", actualEnd: new Date() })
